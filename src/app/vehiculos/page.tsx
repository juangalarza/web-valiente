...
export default function VehiculosPage() {
  ...
  const vehiculos0km = vehiculos.filter(v => v.tipo?.toLowerCase() === 'nuevo');
  const vehiculosUsados = vehiculos.filter(v => v.tipo?.toLowerCase() === 'usado');
    ...
    <div key={v.id} className="model-card reveal visible">
      ...
      <div className="model-body">
        ...
        <div className="model-desc" style={{ marginBottom: '8px' }}>
          ...
          {v.tipo?.toLowerCase() === 'usado' && v.km != null ? ` · ${v.km.toLocaleString('es-AR')} km` : ''}
        </div>
        ...
        <div className="model-foot" style={{ marginTop: 'auto' }}>
          <div>
            <div className="model-label">{v.tipo?.toLowerCase() === 'usado' ? 'Usado' : '0KM'}</div>
      ...
      <section style={{ backgroundColor: 'var(--negro)', paddingTop: '140px', paddingBottom: '60px', textAlign: 'center' }}>
        <div className="section-container">
          ...
          <div style={{ marginTop: '40px', display: 'flex', gap: '8px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <a href="/vehiculos" style={buttonStyle(true)}>TODOS</a>
            <a href="/vehiculos/0km" style={buttonStyle(false)}>0KM</a>
            <a href="/vehiculos/usados" style={buttonStyle(false)}>USADOS</a>
          </div>
        </div>
      </section>

      {loading ? (
        <section style={{ padding: '80px 0', backgroundColor: 'var(--crema)', minHeight: '60vh' }}>
          <div className="section-container" style={{ textAlign: 'center', color: 'var(--gris-40)' }}>
            Cargando vehículos...
          </div>
        </section>
      ) : vehiculos.length === 0 ? (
        <section style={{ padding: '80px 0', backgroundColor: 'var(--crema)', minHeight: '60vh' }}>
          <div className="section-container">
            <div style={{ textAlign: 'center', padding: '100px 20px', color: 'var(--gris-40)', maxWidth: '400px', margin: '0 auto' }}>
              ...
              <h3 style={{ fontSize: '20px', fontWeight: 600, color: 'var(--negro)', marginBottom: '10px' }}>No hay unidades disponibles</h3>
              <p style={{ lineHeight: 1.6 }}>Pronto habrá unidades disponibles.<br/>Consultanos por WhatsApp para más información sobre ingresos.</p>
            </div>
          </div>
        </section>
      ) : (
        <>
            ...
            <section style={{ padding: '80px 0', backgroundColor: 'var(--crema)' }}>
              <div className="section-container">
                <div style={{ marginBottom: '40px' }}>
                  <h2 className="s-title">Últimos <strong><em>0KM</em></strong></h2>
                </div>
                <div className="models-grid">
                  {vehiculos0km.slice(0, 6).map(renderCard)}
                </div>
                {vehiculos0km.length > 6 && (
                  <div style={{ textAlign: 'center', marginTop: '60px' }}>
                    <a href="/vehiculos/0km" className="btn-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'var(--rojo)' }}>
                      Ver todos los 0KM
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3">
                        <path d="M5 12h14M12 5l7 7-7 7" />
                      </svg>
          ...
          {vehiculosUsados.length > 0 && (
            <section style={{ padding: '80px 0', backgroundColor: 'var(--blanco)' }}>
              <div className="section-container">
                <div style={{ marginBottom: '40px' }}>
                  <h2 className="s-title">Últimos <strong><em>Usados</em></strong></h2>
                </div>
                <div className="models-grid">
                  {vehiculosUsados.slice(0, 6).map(renderCard)}
                </div>
                {vehiculosUsados.length > 6 && (
                  <div style={{ textAlign: 'center', marginTop: '60px' }}>
                    <a href="/vehiculos/usados" className="btn-primary" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', background: 'var(--rojo)' }}>
